...
- The current `npm run verify:release` run after `605ac54`: web tests (19 files/106 tests), claim tests (11/11), sibling-aware six-document claim validation, pitch-brief retired-claim guard, hosted-flow/owner-aware submission tests (8/8), pilot tests 6/6, sanitized no-result summary, production build, and built-environment scan passed; hosted function allowlist was visibly skipped because `SUPABASE_ACCESS_TOKEN` was absent.
