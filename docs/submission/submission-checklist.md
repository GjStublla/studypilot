...
- [x] Web repository local evidence commits: code baseline `5fcb87e`; current documentation evidence anchor `47d8c96` (including the panel-body, auth-success, release-gate, demo-claim, and submission-artifact updates). The worktree still preserves unrelated uncommitted edits listed by `git status --short`.
