...
- [x] Web repository local evidence commits: code baseline `5fcb87e`; current documentation evidence anchor `00ce898` (including the panel-body, auth-success, release-gate, and demo-claim updates). The worktree still preserves unrelated uncommitted edits listed by `git status --short`.
