...
- The current `npm run verify:release` run after `6bcb1b9`: web tests (19 files/106 tests), claim tests (11/11), sibling-aware six-document claim validation, pitch-brief retired-claim guard, owner-aware submission tests (7/7), production build, and built-environment scan passed; hosted function allowlist was visibly skipped because `SUPABASE_ACCESS_TOKEN` was absent.
...
- Submission-artifact structure: `node --test scripts/validate-submission-package.test.mjs` (7/7) and `npm run validate:submission` passed the nine-section report, seven-segment 1:58 demo timeline, fallback instructions, checklist markers, and pending-gate ownership annotations. Strict `--require-final-inputs` remains intentionally red with eight human-owned inputs pending.
