...
1. Keep `Dashboard.tsx` orchestration-only and reduce it below the plan's 1,000-line target. It is currently about 1,306 lines after the request-state guards; CSS boundaries are now split by `e1632fe`, so remaining work is orchestration/helper extraction.
