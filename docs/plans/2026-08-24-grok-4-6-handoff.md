...
- Web commit `6213d97` adds `docs/submission/team-contributions-template.md`, which keeps member roles, approvals, mentor/team sign-off, and credential handling human-owned. The submission validator now checks its safety markers; the focused submission suite is 9/9 and `npm run validate:submission` still reports eight real final inputs pending.
...
- The latest local submission slice `6213d97` extends that gate with the human-owned contribution/sign-off template; `node --test scripts/validate-submission-package.test.mjs` now passes 9/9 and the strict final-input command remains intentionally red for the same eight human-owned inputs.
