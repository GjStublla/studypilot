...
- Web commits `3212aca`, `d5657d8`, `c970bd0`, `de9500d`, and `ad3a5f4` add direct expired-session refresh/redirect, network-failure, login-error, OAuth-error, successful email-login, auto-confirmed-signup, and email-confirmation-signup characterization, keep the tests compatible with the repository's Testing Library typings, guard demo-script claims, and restore keyboard tab access to both password-visibility buttons. The current web suite is 19 Vitest files / 106 tests; web Playwright remains 4/4.
...
- Public-claim drift is now guarded by `scripts/validate-claims.mjs` and its Node tests. The local two-repository command `npm run validate:claims -- --extension-root ../studypilot-extension --require-extension` checks six documents: web README, landing/legal copy, final report, canonical extension README, and the demo script (the script is checked for retired claims while its required wording remains human-owned). It rejects retired tab-audio, exact-second, device-only, and no-account claims. The final pitch is intentionally not auto-marked because it is human-owned.
...
- `npm run verify:release` with approved public placeholders after `ad3a5f4` (the follow-up to `de9500d`): web tests (19 files/106 tests), claim tests (8/8), six-document claim validation, production build, and built-env passed; hosted allowlist explicitly skipped because `SUPABASE_ACCESS_TOKEN` is absent.
