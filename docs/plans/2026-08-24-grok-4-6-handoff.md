...
- Follow-up extension commit `219c44d` extracts pure study-session creation and fallback-transcript utilities into `src/content/studySession.ts` with three focused tests. `0b05279` then extracts deterministic voice selection and the browser speech-utterance adapter into `src/content/speech.ts` with four focused tests. The latest `4ce2fce` makes pause/resume presentation and local fallback transitions derive from the canonical live state; `FloatingStudyPilot.tsx` is now 1,433 lines. The canonical extension worktree is clean at `4ce2fce`; full verification is Vitest 20 files/97 tests, typecheck/build/manifest validation, and unpacked Playwright 15/15. Hosted Vertex Live and teammate review remain external.
