...
- Follow-up extension commit `219c44d` extracts pure study-session creation and fallback-transcript utilities into `src/content/studySession.ts` with three focused tests. The later `0b05279` slice extracts deterministic voice selection and the browser speech-utterance adapter into `src/content/speech.ts` with four focused tests; `FloatingStudyPilot.tsx` is now 1,434 lines. The canonical extension worktree is clean at `0b05279`; full verification is Vitest 20 files/95 tests, typecheck/build/manifest validation, and unpacked Playwright 15/15. Hosted Vertex Live and teammate review remain external.
