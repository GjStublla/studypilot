...
- Web commits `3212aca`, `d5657d8`, and `c970bd0` add direct expired-session refresh/redirect, network-failure, login-error, OAuth-error, successful email-login, auto-confirmed-signup, and email-confirmation-signup characterization, keep the tests compatible with the repository's Testing Library typings, and restore keyboard tab access to both password-visibility buttons. The current web suite is 19 Vitest files / 106 tests; web Playwright remains 4/4.
...
- `npm run verify:release` with approved public placeholders after `c970bd0`: web tests (19 files/106 tests), claim tests, five-document claim validation, production build, and built-env passed; hosted allowlist explicitly skipped because `SUPABASE_ACCESS_TOKEN` is absent.
