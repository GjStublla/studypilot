...
2. Add explicit request-state unions for the remaining dashboard fetches and add stale-promise/unmount cleanup tests; `DashboardBootstrapState` now centralizes `idle | loading | success | error` in `9dd63b2`, `d2cf801` applies the same state boundary to per-session transcript loading/error/retry, and `a989db6` ignores superseded/unmounted transcript responses.
...
2. Continue focused body composition from `FloatingStudyPilot.tsx`; `useDashboardWorkspace.ts`, `QuickActions.tsx`, `ExtensionPanel.tsx`, `ChatSwitcher.tsx`, `ContextSettings.tsx`, `StudyModePanel.tsx`, and `AnswerCardPanel.tsx` are now extracted. The remaining parent is approximately 1,952 lines and still owns the main body render composition.
