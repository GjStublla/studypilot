...
- Follow-up extension commit `bf073bf` adds keyboard activation coverage for the first secondary menu action. The canonical extension worktree is clean at `bf073bf`; full verification is now Vitest 18 files/88 tests, typecheck/build/manifest validation, and unpacked Playwright 15/15. Hosted Vertex Live and teammate review remain external.
