...
- The claim gate now includes the time-coded demo script via `--include-demo-script --require-demo-script`: its eight Node tests cover CLI parsing, document-specific required rules, forbidden-claim detection, and real file loading. `npm run validate:claims -- --extension-root ../studypilot-extension --require-extension` passed six documents; hosted/demo/pilot evidence is still not inferred.
