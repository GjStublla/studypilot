...
- Web commit `3212aca` adds direct API/auth characterization: expired sessions redirect after a failed refresh, successful refresh retries the original request, network failures propagate, login/OAuth failures render recoverable alerts, and both password-visibility controls are keyboard reachable. Current web verification: Vitest 19 files/103 tests, production build, built-environment scan, and Playwright 4/4.
