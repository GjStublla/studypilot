...
- Web working slice adds `scripts/validate-claims.mjs` plus Node tests and a CI/release hook. The sibling-aware local command checks five documents (web README, landing/legal copy, final report, and canonical extension README), passes the aligned wording, and rejects retired tab-audio, exact-second, device-only, and no-account claims. The pitch remains human-owned.
