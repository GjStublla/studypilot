...
- The contribution/sign-off preparation slice adds `docs/submission/team-contributions-template.md` with explicit member approval, no-inferred-percentages, mentor/team sign-off, and credential-safe handling. `scripts/validate-submission-package.mjs` and its focused suite now require those markers (9/9); the template remains preparation only and the strict final-input gate remains pending.
