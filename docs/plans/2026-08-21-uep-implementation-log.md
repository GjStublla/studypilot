...
### Phase 8B dashboard data-lifecycle extraction — 2026-08-24

- Commit `0cf07d5` moved dashboard data fetching, profile/bootstrap loading, Supabase Realtime reconciliation, focus/visibility refresh, request-version refs, and chat invalidation behind the typed `useDashboardData` hook. The extension-help modal moved to its own component as a second low-coupling boundary.
- `Dashboard.tsx` fell from approximately 1,306 lines to 938 lines. The hook preserves the existing `DashboardRequestState` unions and mounted/superseded guards; no user-facing behavior or product claim changed.
- Verification: `npx tsc --noEmit`, full web Vitest (17 files / 96 tests), and the production-style release gate passed with approved public HTTPS placeholders. The release gate's hosted Supabase function allowlist remained explicitly **SKIPPED** because `SUPABASE_ACCESS_TOKEN` was not set.
...
- Canonical extension commit `6630915` hydrates Live status on panel mount and adds a deterministic delayed-token Playwright race: start Live, unmount/reopen the panel, stop Live, release the delayed start failure, and assert no stale error or page/extension-page console error. Extension verification after the slice: 17 Vitest files / 84 tests, typecheck, build, manifest validation, and unpacked Playwright 12/12 passed. The test is local transport characterization; hosted Vertex Live remains an external gate.
- Canonical extension commit `397c682` extracts the Pomodoro picker and weekly progress chart into a typed `PomodoroPicker` component, reducing `FloatingStudyPilot.tsx` by 80 lines without changing timer ownership. Extension Vitest 17/84, typecheck, build, manifest validation, and Playwright 12/12 remain green.
- Canonical extension commit `6138fb0` extracts the selection tooltip and its three action buttons into a typed component, reducing the parent by another 86 lines while keeping selection listeners and action callbacks in the parent. Vitest 17/84, typecheck, build, manifest validation, and Playwright 12/12 remain green.
- Web commit `d694163` fixes a fresh-clone reproducibility defect: Supabase JS now receives a non-routable public fallback when `.env` is absent, so browser-client imports remain loadable while the production Vite gate still rejects missing public variables. The isolated clean-clone audit passed npm ci, web Vitest 17/96, Deno 42, public-placeholder build plus built-env scan, backend pytest 26, Supabase pgTAP 5 files/291 after a fresh start, and web Playwright 4/4. README clean-clone commands were added; bare production build failure without public variables remains intentional.
- Phase 13 preparation adds `scripts/validate-pilot-results.mjs` and Node tests. The validator accepts the header-only template without claiming a pilot, rejects identifying/credential/draft values and impossible citation counts, and computes fixed metrics only from complete anonymous rows. The README, pilot protocol, pilot summary, checklist, and evidence matrix now point to `npm run validate:pilot` and its explicit `--require-data` gate.
- Pilot reporting preparation now includes `npm run summarize:pilot`, which emits a sanitized Markdown aggregate draft with denominators and no participant content; the release wrapper and CI validate the empty template without claiming pilot evidence.
- Fresh committed-head clone `C:\Users\gjins\Desktop\studypilot-clean-clone-20260824-pilot-summary` at `605ac54` reproduced npm ci with 0 vulnerabilities, pilot tests 6/6, the no-result summary, claims 11/11, pitch validation, hosted-flow/submission tests 8/8, the full non-hosted release wrapper, and web Playwright 4/4. The clone remained clean; the first bare-build attempt failed closed as designed, then the documented public-placeholder rerun passed.
- Web commit `6213d97` adds the contribution/sign-off preparation slice: `docs/submission/team-contributions-template.md` has explicit member approval, no-inferred-percentages, mentor/team sign-off, and credential-safe handling. `scripts/validate-submission-package.mjs` and its focused suite now require those markers (9/9); the template remains preparation only and the strict final-input gate remains pending.
- Web commit `cc23329` adds `docs/submission/teammate-reproduction-record.md`, which separates exact-commit README walkthrough evidence from hosted/approval evidence. The submission validator now requires its non-claiming safety markers; focused submission tests pass 10/10 and the actual independent walkthrough remains external.
- Fresh committed-head clone `C:\Users\gjins\Desktop\studypilot-clean-clone-20260824-teammate-record` at `c39732c` reproduced `npm ci` with 0 vulnerabilities, submission tests 10/10, the public-placeholder non-hosted release wrapper, and web Playwright 4/4; the clone remained clean. This is local reproduction evidence, not teammate sign-off.
- Phase 13 evidence hardening defines before/after rubric scores as normalized 0–100 values and makes the pilot validator reject out-of-range scores. Focused pilot-validator tests pass 6/6, and the checked-in header-only template still validates without claiming participant evidence.
- The pilot result formatter now labels whether the approved row count is within the protocol's 10–15 target without blocking smaller samples; deviations must be stated as limitations.
- Canonical extension commit `a77fe49` adds a CDP shadow-root focus helper and an unpacked Playwright test for Enter activation of the launcher, settings, and minimize controls. Extension typecheck, Vitest 17 files/84 tests, build, manifest validation, and Playwright 13/13 passed; a broader manual keyboard review of secondary actions and all hosted behavior remain open.
- Canonical extension commits `dd7a2c3` and `db5ff45` add a closed-shadow-root audit for visible control names, tab reachability, and text clipping at 360px and 390px. Extension typecheck, Vitest 17 files/84 tests, build, manifest validation, and unpacked Playwright 14/14 passed; hosted behavior and teammate review remain external.
- Canonical extension commit `b182624` extracts the panel-body render composition into typed `src/content/PanelBody.tsx`; `FloatingStudyPilot.tsx` drops from approximately 1,794 to 1,660 lines while retaining live/workspace state and async ownership. Typecheck, Vitest 17 files/84 tests, build, manifest validation, and unpacked Playwright 14/14 passed.
- Canonical extension commit `4783764` adds a rendered `VoiceDock` characterization: pause is disabled while idle/starting, enabled with the “Pause session” label only when Live is active, and relabeled/enabled as “Resume session” when paused. Extension verification after the slice: Vitest 18 files/88 tests, typecheck, build, manifest validation, and unpacked Playwright 14/14 passed; hosted Vertex Live remains external.
- Web working slice adds `scripts/validate-claims.mjs` plus Node tests and a CI/release hook. The sibling-aware local command checks six documents (web README, landing/legal copy, final report, canonical extension README, and the demo script), applies the full alignment rules to public product copy, and applies the retired-claim guard to the human-owned demo script. It rejects tab-audio, exact-second, device-only, and no-account claims; the final pitch remains human-owned.
- Verification after the claims slice: `node --test scripts/validate-claims.test.mjs` (5/5), web Vitest (19 files/106 tests), and `npm run verify:release` with public placeholders passed tests, claims, production build, and built-environment scan. Hosted function allowlist remained visibly skipped because `SUPABASE_ACCESS_TOKEN` was absent.
- Follow-up commit `785ceb7` makes `verify:release` run the claim unit tests before the validator, so the release wrapper cannot silently accept a regressed claim gate.
- Web commits `3212aca` and `d5657d8` add direct API/auth characterization: expired sessions redirect after a failed refresh, successful refresh retries the original request, network failures propagate, login/OAuth failures render recoverable alerts, successful email login and auto-confirmed signup land on the dashboard, email-confirmation signup remains recoverable, and both password-visibility controls are keyboard reachable. Current web verification: Vitest 19 files/106 tests, production build, built-environment scan, and Playwright 4/4.
- Follow-up `c970bd0` replaces an unsupported Testing Library `exact` option in the new auth-transition tests with anchored accessible-name matchers. Focused auth tests passed 7/7, `npx tsc --noEmit` passed, and the complete `npm run verify:release` rerun passed web Vitest 19 files/106 tests, claim tests/validation, production build, and built-environment scan; hosted function allowlist remained visibly skipped without `SUPABASE_ACCESS_TOKEN`.
- The claim gate now includes the time-coded demo script via `--include-demo-script --require-demo-script`: its eight Node tests cover CLI parsing, document-specific required rules, forbidden-claim detection, and real file loading. Follow-up `ad3a5f4` makes `--require-demo-script` imply inclusion for direct callers. The final `npm run verify:release` rerun at `ad3a5f4` passed web Vitest 19/106, claim tests 8/8, six-document validation, production build, and built-environment scan; hosted/demo/pilot evidence is still not inferred.
- The submission-package gate adds `scripts/validate-submission-package.mjs` and six Node tests. It verifies the exact nine report headings, all seven time ranges ending at 1:58, text-input/backup instructions, and checklist markers; default release/CI mode reports eight pending human-owned inputs, while `--require-final-inputs` correctly fails until those inputs are supplied.
- Web commit `14395cd` adds an ownership traceability gate for pending human-owned checklist items. The checklist now assigns owner roles for deployment/store access, demo/media, pilot, credential, claims, team contribution, and mentor sign-off work; the validator rejects an unowned pending item while keeping the eight inputs pending until real evidence and approvals are supplied. Submission tests pass 7/7 and sibling-aware claims validation remains green.
- After `14395cd`, the full non-hosted `npm run verify:release` wrapper passed again: web Vitest 19/106, claim tests 8/8 and six-document validation, submission tests 7/7 and structural validation, public-placeholder build, and built-environment scan. The hosted allowlist remained an explicit skip because `SUPABASE_ACCESS_TOKEN` was not set.
- Web commit `c8f6594` removes current-facing evidence drift: the report, checklist, and judging matrix now show the 7/7 ownership-aware submission tests and the latest clean-clone path instead of stale 6/6 counts. The full release wrapper passed before this documentation-only commit; no hosted or pilot claim was added.
- The communication phase adds `docs/submission/pitch-claims-brief.md`, a human-owned problem/mechanism/evidence/limitations/Q&A draft. `scripts/validate-claims.mjs` now supports a required pitch-brief retired-claim check through `npm run validate:pitch`; claim tests pass 11/11. This guards preparation copy without treating it as the approved final pitch.
- Fresh committed-head clone `C:\Users\gjins\Desktop\studypilot-clean-clone-20260824-pitch-guard` at `6bcb1b9` reproduced `npm ci` with 0 vulnerabilities, claim tests 11/11, the sibling-aware claims and pitch guards, owner-aware submission tests 7/7, the full non-hosted release wrapper, and web Playwright 4/4. The clone remained clean; hosted allowlist verification stayed explicitly skipped.
- Phase 14 preparation adds `docs/submission/hosted-golden-flow-checklist.md`, which defines the two fresh-profile runs, A1–A8 checkpoints, privacy/fallback rules, evidence categories, and deployment/demo/team sign-offs. It is an execution aid only; no hosted run, account, recording, or pilot result was created.
- The submission validator now checks the hosted-flow checklist markers alongside report/demo/checklist structure; focused submission tests pass 8/8 while the strict final-input gate remains red.
- Fresh committed-head clone `C:\Users\gjins\Desktop\studypilot-clean-clone-20260824-hosted-flow` at `7e88e6e` reproduced npm ci with 0 vulnerabilities, submission tests 8/8, claim tests 11/11, pitch validation, the full non-hosted release wrapper, and web Playwright 4/4. The clone remained clean; hosted allowlist verification stayed explicitly skipped.
- Web commit `823172d` wires the submission-artifact tests and validator into `verify:release`, web CI, and the clean-clone README command sequence. The final release-wrapper run passed web 19/106, claim tests 8/8, submission tests 6/6, structural validation, build, and built-env; the hosted allowlist remained an explicit skip.
- Read-only GitHub inspection confirms both repository `main` branches are currently unprotected and both remotes have no visible workflow runs; this is recorded as a push/admin gate rather than inferred CI evidence.
- Fresh clone `studypilot-clean-clone-20260824-final` reproduces the current committed-head web gates: npm ci with 0 vulnerabilities, Vitest 19/106, Deno 42, six-document claim validation, submission tests/validation, pilot-template validation, public-placeholder build/scan, backend pytest 26, pgTAP 5 files/291 after a clean Supabase start, and web Playwright 4/4. The local Supabase stack was stopped after the run; no hosted claim is inferred.
- The same fresh clone completed the full `npm run verify:release` wrapper with public placeholders. All non-hosted stages passed; the hosted function allowlist printed an explicit skip because `SUPABASE_ACCESS_TOKEN` was absent, so hosted verification remains open.
- Web CI now includes a separate `web-e2e` job that installs Chromium and runs the deterministic Playwright suite; this is workflow evidence only until a pushed commit produces a remote run.
- Canonical extension commit `bf073bf` adds keyboard activation coverage for the first secondary menu action. Full extension verification passed: Vitest 18 files/88 tests, typecheck, build, manifest validation, and unpacked Playwright 15/15; hosted Vertex Live and teammate review remain external.
- Canonical extension commit `1cab20a` extends the connected shared-chat fixture with a second coaching turn after minimize/reopen and page reload; the same active chat is refreshed without duplication or a dropped commit. Full unpacked Playwright remains 15/15; hosted clean-profile behavior remains external.
- Added a read-first, admin-approved branch-protection procedure to `context/runbook.md`, including separate required-check lists for the web and extension repositories. No GitHub settings were changed; protection and remote CI evidence remain external.
- Canonical extension commit `219c44d` extracts `createStudySession` and `fallbackTranscript` into `src/content/studySession.ts`, adds three focused utility tests, and reduces `FloatingStudyPilot.tsx` to approximately 1,610 lines. Full verification passed: Vitest 19 files/91 tests, typecheck, build, manifest validation, and unpacked Playwright 15/15; hosted Live remains external.
- Canonical extension commit `0b05279` extracts deterministic speech voice selection and the browser utterance adapter into `src/content/speech.ts`, adds four focused utility tests, and reduces `FloatingStudyPilot.tsx` to 1,434 lines. Full verification passed: Vitest 20 files/95 tests, typecheck, build, manifest validation, and unpacked Playwright 15/15; hosted Live remains external.
- Canonical extension commit `4ce2fce` makes VoiceDock pause/resume presentation derive from the canonical `LiveUiState`, removes the duplicated paused prop, and aligns null-response pause/resume fallbacks with `live → paused → live`. Full verification passed: Vitest 20 files/97 tests, typecheck, build, manifest validation, and unpacked Playwright 15/15; hosted Live remains external.
- Canonical extension commit `92887aa` rejects pause/resume commands in invalid service-worker states and adds coverage that idle runtime commands do not reach offscreen. Full verification passed: Vitest 20 files/98 tests, typecheck, build, manifest validation, and unpacked Playwright 15/15; hosted Live remains external.
- Phase 0 metadata audit confirms `backend/service-account.json` was added in `1e0f6bb`, removed in `0cd0d99`, and is absent from the current working tree. Key validity/revocation and any coordinated history rewrite remain human-owned; no credential contents were read or printed.
- Current web worktree audit after the extension hardening: Vitest 19 files/106 tests, public-placeholder production build, `verify:built-env`, and Playwright 4/4 passed. The pre-existing web edits were preserved and not staged or rewritten.
- The complete current web `npm run verify:release` wrapper also passed: Vitest 19/106, claim tests 8/8 and six-document validation, submission tests 6/6 and structural validation, production build, and built-env scan. It explicitly skipped only the hosted function allowlist because `SUPABASE_ACCESS_TOKEN` was absent; this is not hosted evidence.
- The preserved web worktree audit found no unfinished feature diff: the visible changes are one indentation-only edit in `Dashboard.tsx` and trailing-newline cleanup in `tsconfig.json`/`tsconfig.node.json`; the other marked files have no textual diff. Keep the status visible and do not reset or stage it without the owner's direction.
- Report commit `525ec45` adds a local evidence snapshot to section 4 of `docs/submission/final-report-content.md`, including the current web/extension/database/browser counts and explicit non-hosted limitations. Claims tests/validation, submission tests/validation, and the full `verify:release` wrapper passed after the report update; the hosted allowlist remained visibly skipped.
- The full web Playwright suite then passed 4/4 twice consecutively from fresh Playwright contexts after `3212aca`. The deterministic fixture remains explicitly local; hosted clean-profile and production smoke evidence are still external.
- Fresh committed-head clone `C:\Users\gjins\Desktop\studypilot-clean-clone-20260824-latest` at `ceda67f` reproduced the updated local gates: `npm ci` (0 vulnerabilities), pilot validator tests 6/6 plus empty-template validation, `verify:release` (web 19/106, claims 8/8, submission tests 6/6, public-placeholder build and built-env), web Playwright 4/4, backend pytest 26, Deno 42, and local Supabase pgTAP 291. The clone remained clean and the local Supabase stack was stopped afterward; hosted availability and remote CI remain unproven.
- Fresh committed-head clone `C:\Users\gjins\Desktop\studypilot-clean-clone-20260824-submission-owners` at `97549c0` reproduced the ownership-aware submission gate: `npm ci` (0 vulnerabilities), submission tests 7/7, sibling-aware claims validation, `verify:submission` with eight explicitly owned pending inputs, and the full non-hosted `verify:release` wrapper. The clone remained clean; hosted allowlist verification was skipped without protected credentials.
- Phase 14 preparation adds `docs/submission/demo-fixture.md` with synthetic rubric, weak paragraph, prompt sequence, action-item text, privacy setup, and visible checkpoints. It is content-only; no demo account or credentials were created.
