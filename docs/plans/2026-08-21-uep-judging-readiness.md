...
19/91, extension Playwright 15/15, production builds, and manifest/built-env
...
**Follow-up evidence — 2026-08-24:** `6630915` hydrates the panel from `STUDYPILOT_GET_LIVE_STATUS` on mount and adds a deterministic delayed-token Playwright scenario covering Live start, panel unmount/reopen, Live stop, delayed start failure suppression, and page/extension-page console-error assertions. `397c682` then extracts the Pomodoro picker and weekly progress chart, `6138fb0` extracts the selection tooltip into typed components, `b182624` moves the complete panel-body render into a typed `PanelBody.tsx` boundary, `4783764` characterizes the rendered VoiceDock pause/resume disabled states for idle, starting, live, and paused sessions, `bf073bf` adds keyboard activation coverage for the first secondary menu action, `1cab20a` repeats a connected coaching turn after reload on the same active chat, `219c44d` extracts pure study-session utilities with focused tests, `0b05279` extracts deterministic speech voice selection plus the browser utterance adapter with focused tests, `4ce2fce` centralizes live pause/resume presentation and null-response fallback transitions on the canonical live state, and `92887aa` rejects invalid pause/resume commands in the service worker with privacy-suite coverage. These slices reduce the parent or tighten the live-control/accessibility/repeatability contract without changing timer, selection-listener, action, live, or workspace ownership. This is local transport characterization only; a real hosted Vertex Live session remains an external gate.
